...
const int PUBLISH_FREQUENCY = 300000; // Update rate in milliseconds
unsigned long timer;
...
void loop() {
  ...
  if(loopVarredura < 9 ){
    ...
    if ((millis() - timer) > PUBLISH_FREQUENCY){
      ...
      lcd_i2c.setCursor(0,1);
      lcd_i2c.print(localVarredura);
      lcd_i2c.setCursor(10,1);
      lcd_i2c.printf(" %2i db",mainSignalValue);
      //---------------------------------------//
      ...
      lcd_i2c.setCursor(0,0);
      lcd_i2c.printf("%1i - VA PARA",loopVarredura);
      lcd_i2c.setCursor(0,1);
      lcd_i2c.print(localVarredura);
      delay(5000);
    }else{
      ...
      lcd_i2c.setCursor(0,0);
      lcd_i2c.printf("Faltam %3.2f seg.\n", ((float)PUBLISH_FREQUENCY - (float)(millis() - timer))/1000.0  );
      }    
    }
  }else{
    mainSignalValue = WiFi.RSSI() + 70;
